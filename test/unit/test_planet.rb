...
class TestPlanet < Test::Unit::TestCase
  ...
  should "return right rover position on a map when a rover moves" do
    @rover.receive_signal('M')

    assert_equal @planet.plan[3], [@rover, nil, nil, nil, nil]
  end

  should "return right rover position on a map when a rover turns" do
    @rover.receive_signal('L')

    assert_equal @planet.plan[4], [@rover, nil, nil, nil, nil]
  end

  should "return right rover position on a map after a longer trip" do
  ...
  should "properly prepare a nice plan for printing" do
